...
const Auth: FC<TAuthProps> = ({ setIsOpen }) => {
  ...
  return (
    <section
      ...
      onClick={(e) =>
        !(e.target as HTMLElement).closest("#auth-body") && setIsOpen(false)
      }
    >
      <div id="auth-body" className={styles.auth__body}>
        <nav className={styles.auth__navigation}>
          {allInfo.step && (
            <button
              onClick={() => setAllInfo((prev) => ({ ...prev, step: false }))}
              type="button"
            >
              <IoMdArrowBack />
            </button>
        ...
        {!allInfo.step && (
          <AuthSendForm allInfo={allInfo} setAllInfo={setAllInfo} />
        )}
        {allInfo.step && (
          <AuthKeyForm allInfo={allInfo} setAllInfo={setAllInfo} />
        )}
      </div>
    </section>
  );
